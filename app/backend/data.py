...
# ── Data Quality Logger ──────────────────────────────────────────────────────
_dq_log = logging.getLogger("data_quality")
if not _dq_log.handlers:
    _dq_log.setLevel(logging.INFO)
    _backend_log = Path(__file__).parent / "backend.log"
    _h = logging.FileHandler(_backend_log)
    _h.setFormatter(logging.Formatter("%(asctime)s [DQ] %(levelname)s %(message)s"))
    _dq_log.addHandler(_h)
    # Also surface to console
    _ch = logging.StreamHandler()
    _ch.setFormatter(logging.Formatter("[DQ] %(levelname)s %(message)s"))
    _dq_log.addHandler(_ch)

# Public status object — readable via get_data_quality_status() for /health endpoint
DATA_QUALITY_STATUS = {
...
# Repair config (mirrors validator thresholds)
_TRIP_COUNT_RANGE = (0, 5000)
_HOLIDAY_RUN_MAX = 3
_LAG1W_NEW_CORR_MIN = 0.20  # was 0.50
_LAG1W_DROP_MIN = 0.50  # was 0.30
_HISTORICAL_CUTOFF = pd.Timestamp("2026-01-16")


def _apply_data_quality_fixes(df: pd.DataFrame, *, label: str) -> pd.DataFrame:
    """
    Apply graceful-degradation fixes to a freshly loaded DataFrame.
    Never raises; logs each repair through _dq_log and updates DATA_QUALITY_STATUS.
    Operates only on columns that exist in df, so it's safe for partial-column reads.

    Order matters:
      1. Snapshot raw df for the lag check (must see same data as validator).
      2. Duplicates first — they inflate every downstream stat.
      3. lag_1week check — uses the pristine snapshot, repairs the working df.
      4. trip_count clip — fixes negatives and absurd highs.
      5. is_holiday last — purely a flag override, doesn't affect other columns.
    """
    if df is None or df.empty:
        _dq_log.warning("[%s] empty dataframe — skipping DQ repairs", label)
        return df

    n_in = len(df)
    fallbacks: list[str] = []

    # 0) Snapshot for the lag check — must match validator's raw-data view
    df_original = (
        df.copy()
        if {"lag_1week", "trip_count", "PULocationID", "time_bucket"}.issubset(
            df.columns
        )
        else None
    )
    ...
    if {"PULocationID", "time_bucket"}.issubset(df.columns):
        before = len(df)
        df = df.drop_duplicates(subset=["PULocationID", "time_bucket"], keep="last")
        dropped = before - len(df)
        if dropped:
            DATA_QUALITY_STATUS["rows_dropped"] += dropped
            fallbacks.append(f"duplicates:dropped={dropped}")
            _dq_log.warning(
                "[%s] dropped %d duplicate (zone, time_bucket) rows", label, dropped
            )

    # 2) lag_1week cross-contamination — detect on raw snapshot, repair on working df
    if df_original is not None:
        hist = df_original[
            (df_original["time_bucket"] < _HISTORICAL_CUTOFF)
            & df_original["lag_1week"].notna()
        ]
        new = df_original[
            (df_original["time_bucket"] >= _HISTORICAL_CUTOFF)
            & df_original["lag_1week"].notna()
        ...
        if not hist.empty and not new.empty:
            ...
            hc = _per_zone_corr(hist)
            nc = _per_zone_corr(new)
            common = hc.index.intersection(nc.index)
            if len(common):
                drops = hc.loc[common] - nc.loc[common]
                broken = common[
                    (nc.loc[common] < _LAG1W_NEW_CORR_MIN) & (drops >= _LAG1W_DROP_MIN)
                ]
                if len(broken) and "zone_slot_baseline" in df.columns:
                    repair_mask = df["PULocationID"].isin(broken)
                    df.loc[repair_mask, "lag_1week"] = df.loc[
                        repair_mask, "zone_slot_baseline"
                    ].astype(df["lag_1week"].dtype)
                    DATA_QUALITY_STATUS["lag_repaired_zones"] = sorted(
                        int(z) for z in broken
                    )
                    fallbacks.append(f"lag_1week_repaired:{list(map(int, broken))}")
                    _dq_log.warning(
                        "[%s] lag_1week broken for zones %s; replaced with zone_slot_baseline",
                        label,
                        list(map(int, broken)),
                    )

    # 3) Out-of-range trip_count
    if "trip_count" in df.columns:
        lo, hi = _TRIP_COUNT_RANGE
        bad_mask = (df["trip_count"] < lo) | (df["trip_count"] > hi)
        n_bad = int(bad_mask.sum())
        if n_bad:
            df.loc[bad_mask, "trip_count"] = df.loc[bad_mask, "trip_count"].clip(lo, hi)
            DATA_QUALITY_STATUS["rows_clipped"] += n_bad
            fallbacks.append(f"trip_count_clipped={n_bad}")
            _dq_log.warning(
                "[%s] clipped %d trip_count values to [%d, %d]", label, n_bad, lo, hi
            )

    # 4) is_holiday stuck — detect runs > threshold and override to 0
    if {"is_holiday", "time_bucket"}.issubset(df.columns):
        ...
        if not stuck.empty:
            ...
            bad_runs = runs[runs["n"] > _HOLIDAY_RUN_MAX]
            for _, r in bad_runs.iterrows():
                window_mask = (df["time_bucket"] >= r["start"]) & (
                    df["time_bucket"] < r["end"] + pd.Timedelta(days=1)
                )
                n_overrides = int((df.loc[window_mask, "is_holiday"] == 1).sum())
                df.loc[window_mask, "is_holiday"] = 0
                DATA_QUALITY_STATUS["holiday_overrides"] += n_overrides
                fallbacks.append(
                    f"holiday_run_overridden:{r['start'].date()}→{r['end'].date()}"
                )
                _dq_log.warning(
                    "[%s] is_holiday run too long (%d days, %s→%s); overrode %d rows to 0",
                    label,
                    int(r["n"]),
                    r["start"].date(),
                    r["end"].date(),
    ...
    DATA_QUALITY_STATUS["fallbacks_applied"].extend(fallbacks)
    _dq_log.info(
        "[%s] DQ pass complete — input=%d, output=%d, fixes=%d",
        label,
        n_in,
        len(df),
        len(fallbacks),
...
def get_data_quality_status() -> dict:
    """Public accessor for the API/health endpoint."""
    return dict(DATA_QUALITY_STATUS)
...
def _load():
    ...
    try:
        ...
        df["time_bucket"] = pd.to_datetime(df["time_bucket"])
        df = _apply_data_quality_fixes(df, label="profile_load")
        DATA_QUALITY_STATUS["load_succeeded"] = True
    except Exception as e:
        _dq_log.error(
            "Profile load failed catastrophically: %s — returning empty profile", e
        )
        return pd.DataFrame(
            columns=["PULocationID", "hour", "dayofweek", "avg", "holiday_name"]
        ), pd.read_csv(LOOKUP_PATH).rename(
            columns={
                "LocationID": "zone_id",
                "Zone": "name",
                "Borough": "borough",
...
def _load_full_demand():
    """Load full demand data for lag calculation. Never raises — returns None on hard failure."""
    print("[NYC Cab Analytics] Loading full demand data for forecasting...")
    try:
        df = pd.read_parquet(DATA_PATH)
        df["time_bucket"] = pd.to_datetime(df["time_bucket"])
        df = _apply_data_quality_fixes(df, label="full_demand_load")
        return df
    except Exception as e:
        _dq_log.error("Full demand load failed: %s — forecasting disabled", e)
        return None
