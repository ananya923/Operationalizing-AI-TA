...
class TestToolRouting:
    ...
    def test_policy_query_calls_search_policies(self):
        tools, context, _, _ = run_tools(
            "policy", "What is the travel policy?", "engineer", "test_user"
        )
    ...
    def test_employee_query_with_name(self):
        ...
        tools, context, _, _ = run_tools(
            "employee", "Who is Richard in Finance?", "manager", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_expense_query_finance_role(self):
        tools, context, _, _ = run_tools(
            "expense", "What is the total spending?", "finance", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_expense_query_category_breakdown(self):
        tools, context, _, _ = run_tools(
            "expense", "Show expense breakdown by category", "finance", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_project_budget_query(self):
        tools, context, _, _ = run_tools(
            "project", "Show me budget status of all projects", "manager", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_project_active_query(self):
        tools, context, _, _ = run_tools(
            "project", "Which projects are active?", "engineer", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_benefits_pto_query(self):
        tools, context, _, _ = run_tools(
            "benefits", "What is the average PTO per year?", "hr", "test_user"
        )
        tool_names = [t.tool for t in tools]
    ...
    def test_benefits_health_query(self):
        tools, context, _, _ = run_tools(
            "benefits", "What health plans are available?", "hr", "test_user"
        )
        tool_names = [t.tool for t in tools]
...
class TestFallbackBehavior:
    ...
    def test_unknown_intent_falls_back_to_policy_search(self):
        ...
        tools, context, _, _ = run_tools(
            "policy", "something completely unrelated", "engineer", "test_user"
        )
    ...
    def test_engineer_expense_query_raises_auth_error(self):
        ...
        with pytest.raises(g.AuthorizationError):
            run_tools("expense", "What is the total spending?", "engineer", "test_user")
    ...
    def test_context_is_string(self):
        _, context, _, _ = run_tools(
            "policy", "What is the PTO policy?", "engineer", "test_user"
        )
        assert isinstance(context, str)

    def test_tools_called_is_list(self):
        tools, _, _, _ = run_tools(
            "policy", "What is the PTO policy?", "engineer", "test_user"
        )
